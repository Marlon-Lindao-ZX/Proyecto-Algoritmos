...
def dijkstra(g, source):
    ...
    archivo = open("C:\\Users\\CORE I7\\Downloads\\tiempos-D.txt", "a", encoding="utf8")
    tiempoinicial = time()
    unvisited = set(g)
    distance = dict.fromkeys(g, float('inf'))
    distance[source] = 0
    ...
    trama = "El tiempo que toma Dijkstra en " +  str(b) + " ejecutarse es de: " + str(tiempo) + "\n"
    archivo.write(trama)
    archivo.close()
...
for a in list:
    value = a
    val2 = random.randrange(value)
    list = []

    for i in range(value):
        key = random.randrange(a*5) + 5
        while (key in g):
            key = random.randrange(a*5) + 5
        g.add_vertex(key)
        list.append(key)

    for k in range(value + val2):
        src = random.randrange(value)
        dest = random.randrange(value)
        weight = random.randrange(20) + 1
        while (g.does_edge_exist(list[src], list[dest]) | src == dest):
            src = random.randrange(value)
            dest = random.randrange(value)
        g.add_edge(list[src], list[dest], weight)
    archivo = open("C:\\Users\\CORE I7\\Downloads\\tiempos-J.txt","a",encoding="utf8")
    tiempoinicial = time()
    distance = johnson(g)
    tiempofinal = time()
    tiempo = tiempofinal - tiempoinicial
    trama = "El tiempo que toma Johnson en ejecutarse con un grafo de: " + str(a) +" es de: " + str(tiempo) + "\n"
    archivo.write(trama)
    archivo.close()
    b += 1
